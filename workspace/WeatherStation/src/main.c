...
volatile FIL fileGotas, filePrecipitacion, fileBackupGotas, fileBackupPrecipitacion;
FIL * punteroBackupGotas;
...
int main(void)
{
	...
	float GAP_VOLTAGE = 0.040; //en volts
	...
	int16_t maxIndice, bytesWritten;
    ...
    f_mount(0, &Fatfs[0]);
    f_open(&fileGotas, "distribucionGotas.txt", 0x13);
    f_open(&fileBackupGotas, "distribucionGotas.backup", 0x13);
    punteroBackupGotas = &fileBackupGotas;
	f_open(&filePrecipitacion, "precipitacionAcumuladaIntensidad.txt", 0x13);
    f_open(&fileBackupPrecipitacion, "precipitacionAcumuladaIntensidad.backup", 0x13);
	...
	while(1)
	{
		 ...
		 if(startReceived)
		 {
			...
			EnviarStartOk(startReceivedUART);
			TransferirDatosSDtoUserInterface(&fileGotas, startReceivedUART, TransmitHistoryGotaDetectada);
			f_close(&fileGotas);
			f_open(&fileGotas, "distribucionGotas.txt", 0x0B);
		 	TransferirDatosSDtoUserInterface(&filePrecipitacion, startReceivedUART, TransmitHistoryVolumenIntesidad);
		 	f_close(&filePrecipitacion);
		 	f_open(&filePrecipitacion, "precipitacionAcumuladaIntensidad.txt", 0x0B);
		 	//duracionSenial = timer0_m0_counter - tiempoInicial;
		 	startReceived = FALSE;
		 	EnviarFinishHistoryTransmission(startReceivedUART);
		 }

		 if(rtcReadRequestReceived)
		 {
			 EnviarRTCReadResponse(rtcReadRequestReceivedUART);
			 rtcReadRequestReceived = FALSE;
		 }

		 if(stopReceived)
		 {
			 EnviarStopOk(stopReceivedUART);
			 stopReceived = FALSE;
		 }
		 ...
		 {
			 alarm_on_XBEE = FALSE;
			 UpdatePrecipitationDataUserInterface(3);
		 }
		 if(alarm_on_RS485)
		 {
			 alarm_on_RS485 = FALSE;
			 UpdatePrecipitationDataUserInterface(0);
		 }
			 ...
			 switch (largoSenial)
			 {
				 ...
				 case 0x03:
					 if( (timer0_m0_counter - auxTiempo[1]) < 10 )
					 {
						 ...
						 //Eliminar senales espurias
						 if(maximo > 2.4 && moduloSenial < 300000)
						 {	 falsaGota = TRUE;
						 }

						 //Eliminar golpes que saturan
						 /*
						 if(maximo == 3.3)
						 {	 falsaGota = TRUE;
						 }*/
						 ...
						 if(!falsaGota)
						 {
							 ...
							 else
							 {
								 ...
								 tramaATransmitir[9+tamanioPayloadApp]= '\n';
								 if(punteroBackupGotas->fsize)
									 f_lseek(punteroBackupGotas, punteroBackupGotas->fsize);
								 f_write(punteroBackupGotas, &tramaATransmitir[9], tamanioPayloadApp+1, &bytesWritten);
								 f_sync(punteroBackupGotas);
